...
def chisel():
    ...
    pg.moveTo(_00[0]+noise,_00[1]+noise+random.randint(-1,3),.7,pg.easeInQuad)
    time.sleep(.5)
    pg.click()
#click inv 1
    pg.moveTo(_01[0]+noise,_03[1]+noise,.5,pg.easeInQuad)
    time.sleep(.5)
    pg.click()
#press space to make
    ...
    pg.press('space')
    time.sleep(15.5+random.random()/2)
...
def mine():
    ...
    pg.click()
    time.sleep(random.randint(10,15) + random.random()/5)
    if random.randint(0,15) == 1:
        # pg.press('f1')
        # pg.click()
...
for i in tqdm(range(num_)):
    ...
    if i % 2 == 0 and i != 0:
        ...
        move_through_inv(RR[1:3],shuffle=False,reverse=True,click=True)
